...
from ..model import DeviceHello, DeviceRtc, DeviceStatus
from ..protocol import ClockProtocol
from ..serial_worker import SerialWorker
from ..widgets import ColorButton, LogPanel, StatusPanel
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _build_connection_bar(self) -> QtWidgets.QWidget:
        group = QtWidgets.QGroupBox("Clock Runtime")
        ...
        note = QtWidgets.QLabel(
            "Calibration and device test controls now live in the separate calibration tool."
        )
        ...
        layout.addWidget(self.open_calibration_button, 0, 5)
        ...
        self.open_calibration_button.clicked.connect(self._open_calibration_tool)
        return group
    ...
    def _build_clock_group(self) -> QtWidgets.QGroupBox:
        ...
        mode_grid.addWidget(self.mode_send_button, 0, 2)
        mode_grid.addWidget(QtWidgets.QLabel("Hour"), 1, 0)
        mode_grid.addWidget(self.hour_mode_combo, 1, 1)
        mode_grid.addWidget(self.hour_mode_send_button, 1, 2)
        mode_grid.addWidget(QtWidgets.QLabel("Colon"), 2, 0)
        mode_grid.addWidget(self.seconds_mode_combo, 2, 1)
        mode_grid.addWidget(self.seconds_mode_send_button, 2, 2)
        layout.addLayout(mode_grid)

        brightness_grid = QtWidgets.QGridLayout()
        ...
        self.colon_brightness_slider = QtWidgets.QSlider(QtCore.Qt.Orientation.Horizontal)
        ...
        brightness_grid.addWidget(QtWidgets.QLabel("Brightness"), 0, 0)
        brightness_grid.addWidget(self.brightness_slider, 0, 1)
        brightness_grid.addWidget(self.brightness_value_label, 0, 2)
        brightness_grid.addWidget(self.brightness_send_button, 0, 3)
        brightness_grid.addWidget(QtWidgets.QLabel("Colon Brightness"), 1, 0)
        brightness_grid.addWidget(self.colon_brightness_slider, 1, 1)
        brightness_grid.addWidget(self.colon_brightness_value_label, 1, 2)
        brightness_grid.addWidget(self.colon_brightness_send_button, 1, 3)
        layout.addLayout(brightness_grid)
        ...
        for index, target in enumerate(BASE_COLOR_TARGETS):
            ...
            button = ColorButton(title, COLOR_PRESETS["Warm Classic"].get(target, (0, 0, 0)))
            self.color_buttons[target] = button
            color_grid.addWidget(button, index // 3, index % 3)
        layout.addLayout(color_grid)

        color_send_row = QtWidgets.QHBoxLayout()
        self.send_basic_colors_button = QtWidgets.QPushButton("Send Base Colors")
        self.send_group_colors_button = QtWidgets.QPushButton("Send Hour / Minute")
        ...
        self.hour_mode_send_button.clicked.connect(
            lambda: self._send_command(ClockProtocol.set_hour_mode(self.hour_mode_combo.currentText()))
        )
        self.seconds_mode_send_button.clicked.connect(
            lambda: self._send_command(ClockProtocol.set_seconds_mode(self.seconds_mode_combo.currentText()))
        )
        self.brightness_send_button.clicked.connect(
            lambda: self._send_command(ClockProtocol.set_brightness(self.brightness_slider.value()))
        )
    ...
    def _build_rtc_timer_group(self) -> QtWidgets.QGroupBox:
        group = QtWidgets.QGroupBox("RTC, Timer, Stopwatch")
        layout = QtWidgets.QVBoxLayout(group)
        layout.setSpacing(8)
        ...
        self.timer_format_send_button = QtWidgets.QPushButton("Send Format")
        ...
        timer_layout.addWidget(self.timer_format_send_button, 1, 3)
        timer_layout.addWidget(self.timer_start_button, 1, 4)
        timer_layout.addWidget(self.timer_stop_button, 1, 5)
        timer_layout.addWidget(self.timer_reset_button, 1, 6)

        self.timer_color_buttons: dict[str, ColorButton] = {}
        for index, target in enumerate(TIMER_COLOR_TARGETS):
            title = target.replace("TIMER_", "").replace("_", " ").title()
            button = ColorButton(title, COLOR_PRESETS["Warm Classic"].get(target, (80, 255, 120)))
            self.color_buttons[target] = button
            self.timer_color_buttons[target] = button
            timer_layout.addWidget(button, 2 + index // 3, index % 3, 1, 2)
        self.timer_colors_send_button = QtWidgets.QPushButton("Send Timer Colors")
        timer_layout.addWidget(self.timer_colors_send_button, 3, 6)
        ...
        stopwatch_layout.addWidget(self.stopwatch_colors_send_button, 1, 3, 1, 3)
        ...
        self.raw_command_edit.setPlaceholderText("Send a line-based command to the runtime firmware")
        ...
        self.read_rtc_button.clicked.connect(lambda: self._send_command(ClockProtocol.rtc_read()))
        ...
        self.timer_format_send_button.clicked.connect(
            lambda: self._send_command(ClockProtocol.set_timer_format(str(self.timer_format_combo.currentData())))
        )
        self.timer_start_button.clicked.connect(lambda: self._send_command(ClockProtocol.timer_start()))
        self.timer_stop_button.clicked.connect(lambda: self._send_command(ClockProtocol.timer_stop()))
        self.timer_reset_button.clicked.connect(lambda: self._send_command(ClockProtocol.timer_reset()))
    ...
    def _on_line_received(self, line: str) -> None:
        ...
        if parsed.kind == "hello":
            ...
            self.device_hello.calibration_mode = bool(parsed.data.get("calibration_mode", self.device_hello.calibration_mode))
            self.device_hello.rtc_present = parsed.data.get("rtc_present", self.device_hello.rtc_present)
            ...
            self.device_status.hour_mode = parsed.data.get("HOURMODE", self.device_status.hour_mode)
            self.device_status.seconds_mode = parsed.data.get("SECONDSMODE", self.device_status.seconds_mode)
            self.device_status.timer_format = parsed.data.get("timer_format", self.device_status.timer_format)
            self.device_status.brightness = int(parsed.data.get("brightness", self.device_status.brightness))
            self.device_status.colon_brightness = int(
                parsed.data.get("colon_brightness", self.device_status.colon_brightness)
            )
            self.device_status.rainbow = bool(parsed.data.get("rainbow", self.device_status.rainbow))
            self.device_status.timer_preset = int(parsed.data.get("timer_preset", self.device_status.timer_preset))
            self.device_status.timer_remaining = int(parsed.data.get("timer_remaining", self.device_status.timer_remaining))
            self.device_status.timer_running = bool(parsed.data.get("timer_running", self.device_status.timer_running))
            self.device_status.stopwatch_running = bool(
                parsed.data.get("stopwatch_running", self.device_status.stopwatch_running)
            )
            ...
            self.device_rtc.lost_power = parsed.data.get("lost_power", self.device_rtc.lost_power)
            self.device_rtc.is_missing = bool(parsed.data.get("is_missing", self.device_rtc.is_missing))
    ...
    def _refresh_status_panel(self, connection_message: str | None = None) -> None:
        ...
        self.status_panel.set_value("current_phys", "Use Calibration Tool")
        self.status_panel.set_value("mapping", "Managed in Calibration Tool")
    ...
    def _set_rtc_from_pc(self) -> None:
        ...
        self._send_command(
            ClockProtocol.rtc_set(now.year, now.month, now.day, now.hour, now.minute, now.second)
        )

    def _set_rtc_manual(self) -> None:
        dt = self.rtc_datetime_edit.dateTime().toPython()
        self._send_command(
            ClockProtocol.rtc_set(dt.year, dt.month, dt.day, dt.hour, dt.minute, dt.second)
        )
